    ...
    firebase.database().ref('estoque').on('value', function (snapshot){
        ...
        snapshot.forEach(function(item){
              var listHtml = '<div class="row block block-strong">';
                ...
                listHtml += '<div class="col-20 fonteestoquejs25">'+ item.val().Fabricante +'</div>';
                listHtml += '<div class="col-60 fonteestoquejs25">'+ item.val().Descricao +'</div>';
                listHtml += '<div class="col-20 fonteestoquejs25">'+ item.val().Preco +'</div>';
